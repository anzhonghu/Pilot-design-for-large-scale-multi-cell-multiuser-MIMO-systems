...
SIR_pick = zeros(sigma_num,4);
count = 0;
for sigmadb = 1 : 1 : sigma_num
    sigma = sig_store(sigmadb,1);
    count = count + 1;
    G = zeros(i_ant,K*L*L);
    for jj = 1 : Num
        ge_sequence1;
...
xx = axes('FontSize',16);
set(h,'PaperType','A4');
plot(sig_store,SIR_pick(:,4),'k-.s','LineWidth',2,'MarkerSize',10)
hold on
plot(sig_store,SIR_pick(:,2),'k-.x','LineWidth',2,'MarkerSize',10)
plot(sig_store,SIR_pick(:,3),'k-.o','LineWidth',2,'MarkerSize',10)
plot(sig_store,SIR_pick(:,1),'k-+','LineWidth',2,'MarkerSize',10)
le = legend('Pilots in [8]','Pilots in [9]','Random pilots','Proposed pilots','Location','SouthWest');
set(le,'Fontsize',16,'Fontname','Times')
xlabel('Shadow fading standard deviation (dB)','Fontsize',20,'Fontname','Times')
ylabel('P \geq 0.95 SIR (dB)','Fontsize',20,'Fontname','Times')
%print(h,'-dpdf','7_cell_SIR_shadow_deviation')
